...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def forward(self, feats):
        ...
        for i in range(self.num_levels):
            ...
            size, bs = (pred_map.shape[2], pred_map.shape[3]), pred_map.shape[0]
            _pred_map = pred_map.permute(0,2,3,1)
            _pred_map = _pred_map.reshape(bs, size[0] * size[1] * self.num_anchors, self.num_attrib)
            _pred_maps.append(_pred_map)
        
        
        _pred_maps = torch.cat(_pred_maps, dim = 1)
        #print(_pred_maps.shape)
        conf = torch.sigmoid(_pred_maps[:, :, 4])
        #print(conf)
        scores = torch.sigmoid(_pred_maps[:, :, 5:])
        #print(conf.shape, scores.shape)
        #print(scores.shape)
        ...
        print(max_score[0], CLASSES[max_index[0]])
        '''
        #sys.exit()    
        
        #pred_maps = torch.cat(pred_maps, dim = 1)
        #conf = pred_maps[:, :, 4]
        #score = torch.sigmoid(pred_maps[:, :, 5:])
        #score = torch.max(score, dim=2)[0]
        #print(conf.shape)
        #print(score.shape)
        #print(max_score.shape, max_index.shape)
