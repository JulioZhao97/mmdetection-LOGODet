...
class LogoDCBBoxHead(nn.Module):
    ...
    def loss(self,
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                cls_score_all = torch.cat(cls_score_all, dim = 1).to(labels.device)
                ...
                for proposal in range(labels.shape[0]):
                    if labels[proposal] != 19:
                        ...
                        cls_score_style.append(cls_score[proposal][labels[proposal] : labels[proposal] + 2].unsqueeze(0))
                style_labels = torch.cat(style_labels, dim=0).to(cls_score.device).long()
                cls_score_style = torch.cat(cls_score_style, dim=0).to(cls_score.device)
                label_weights = torch.ones(style_labels.shape[0]).to(cls_score.device)
                avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
