...
class LOGOConvFCBBoxSHFHead(LogoBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(LOGOConvFCBBoxSHFHead, self).__init__(*args, **kwargs)
    ...
    def init_weights(self):
        super(LOGOConvFCBBoxSHFHead, self).init_weights()
        ...
        if self.training:
            for bs in range(x.shape[0]):
                ...
                if p >= 0.9:
                    ...
                    w, h = random.randint(1, 7), random.randint(1, 7)
                    for i in range(w):
                        for j in range(h):
                            if x1 + i >= 7 or y1 + j >= 7 or x2 + i >= 7 or y2 + j >= 7:
                                continue
                            # exchange
                            tmp = x[bs, :, x1 + i, y1 + j]
                            x[bs, :, x1 + i, y1 + j] = x[bs, :, x2 + i, y2 + j]
                            x[bs, :, x2 + i, y2 + j] = tmp
...
@HEADS.register_module()
class SharedLOGO2FCBBoxSHFHead(LOGOConvFCBBoxSHFHead):

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(SharedLOGO2FCBBoxSHFHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
